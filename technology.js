...
async function func() {
  const domainresponse = await fetch(
    `https://newsapi.org/v2/top-headlines?country=in&category=technology&apiKey=134f5058cd784ef0ab1f0968a3c948f0`
  );
...
async function load() {
   const domainresponse = await fetch(
     `https://newsapi.org/v2/top-headlines?country=us&category=technology&apiKey=134f5058cd784ef0ab1f0968a3c948f0`
   );
   console.log(domainresponse);
   ...
   for (let i = 20; i <22; i++) {
     ...
     div.innerHTML += `<div class="maincontainerbottom1"><img src="${data.articles[i].urlToImage}" alt="">
      <div class="maincontainerbottom2">
        <h4>${data.articles[i].title}</h4>
        <span><b>short</b>&nbspby&nbsp${data.articles[i].author}&nbsp/&nbsp${data.articles[i].publishedAt}</span><a href=${data.articles[i].source.url}  style="text-decoration: none; magin-bottom ="40px"">
                 <p style="margin-bottom="20px": none" class="paragraph">${data.articles[i].description}</p>
